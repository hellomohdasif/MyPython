...
base1 = "http://cmps.unizar.es/sites/cmps.unizar.es/files/webform/"
...
while (i < len(a)):
    driver.get(
        "http://cmps.unizar.es/en/node/20")
    ...
    s3 = WebDriverWait(driver, 8).until(
        EC.element_to_be_clickable((By.ID, "edit-submitted-datos-personales-documento-identidad-archivo-upload")))
    s3.send_keys(path + a[i])

    s4 = WebDriverWait(driver, 8).until(
        EC.element_to_be_clickable((By.XPATH, "//input[@name='op' and @value='Submit']")))
    s4.click()
    ...
    try:
        ...
        men_menu = wait.until(EC.visibility_of_element_located((By.XPATH,
                                                                '//*[@id="block-system-main"]/div/div[3]/a')))
